...
- See `../_shared/references/data-contracts.md` for EvidenceItem schema
...
1. Use **`search` capability** to find relevant results
2. For each promising result, use **`fetch` capability** to extract content
3. If `fetch` capability fails on JavaScript-heavy pages, note the URL for potential adapter-backed browser fallback
...
| Scenario | Action |
|---|---|
| `fetch` capability fails for a URL | Retry once; if still fails, skip with note in audit trail |
